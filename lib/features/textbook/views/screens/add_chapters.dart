...
class _AddChaptersScreenState extends ConsumerState<AddChaptersScreen> {
  ...
  Map<int, List<Map<String, dynamic>>> _sections = {};
  int? _selectedBookId;
  ...
  Future<void> _addSection(int chapterId) async {
    if (_sectionTitleController.text.isNotEmpty) {
      final sectionSlug = _sectionTitleController.text.toLowerCase().replaceAll(' ', '-');
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Section "${_sectionTitleController.text}" added successfully!'),
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
        ),
      );
